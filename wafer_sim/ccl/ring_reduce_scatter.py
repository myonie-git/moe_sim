"""Naive ring ReduceScatter reference implementation."""
...
@CCLAlgorithmRegistry.register("ring_reduce_scatter")
class NaiveRingReduceScatter(CCLAlgorithm):
    """Simple ring ReduceScatter.

    Each round forwards one chunk to the clockwise neighbor while modeling reduction.
    Time complexity is O((N - 1) * num_chunks).
    Expected ideal bandwidth is roughly total_bytes_sent / completion_time.
    """

    SUPPORTED_OPS = ["reduce_scatter"]
